...
Explanation: Full implementation of Grover's search algorithm on a 6-qubit register (64 datas).
...
def step_two_rotate(qc):
    ...
    qc.swap(data[2], data[4])
    qc.swap(data[4], data[6])
    
    qc.swap(data[1], data[3])
    qc.swap(data[3], data[5])
    qc.swap(data[5], data[7])
...
def flip_oracle(qc):
    ...
    """
    Explanation: Conditional ancilla flip used to induce phase kickback on the marked data.
    ...
    qc.mcx([data[0],data[1], data[2], data[3], data[4], data[5]], grover_ancilla)
    qc.z(grover_ancilla)
    qc.mcx([data[0],data[1], data[2], data[3], data[4], data[5]], grover_ancilla)
...
def diffuser(qc):
    """
    Explanation: Applies the Grover diffusion operator, reflecting the data about the uniform superposition.
    ...
    control_qubits = [nonce[0], nonce[1], nonce[2], nonce[3], nonce[4], nonce[5], nonce[6]]
    target_qubit = nonce[7] # One target qubit
